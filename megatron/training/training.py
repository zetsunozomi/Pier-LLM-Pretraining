    ...
    while iteration < args.train_iters:
        ...
        # momentum produce (historical runtime only)
            ...
            if iteration % args.outer_sync_interval == 0 and iteration != starting_iteration:
                ...
                if (outer_shard or dp_rank == 0) and iteration % args.save_interval == 0 and iteration != starting_iteration:
                    # momentum_to_save = [buf.cpu() for buf in momentum_buffer]
                    if momentum_checkpoint_path is not None:
                        torch.save(checkpoint_outer_states(momentum_buffer), momentum_checkpoint_path)
        ...
        # outer lr: 10% lazy start, 10-20% linear warmup, 20%-80% set to 1.1, 80%-100% set to 0.9.
            ...
            if args.outer_optimizer == "pytorch_nesterov":
                ...
                # ============================================================
                # Original miu schedule:
                #   Continuous cosine annealing over the remaining full training:
                #   0.99 -> 0.90 from 10%T to 100%T.
                #
                # This is commented out because we now want miu to reach 0.90
                # already at 20%T, i.e., iteration 20000 when total_steps=100000,
                # and stay at 0.90 afterwards.
                # ============================================================
                ...
                # ============================================================
                # New miu schedule:
                #   Use the same cosine function shape as before.
                #
                #   0.10T -> 0.20T: 0.99 -> 0.90
                #   after 0.20T:    keep 0.90
                #
                # When total_steps = 100000:
                #   iteration 10000: miu = 0.99
                #   iteration 20000: miu = 0.90
                #   iteration >20000: miu = 0.90
                # ============================================================
                _miu_start, _miu_end = 0.99, 0.90
                _miu_decay_start_iter = 0.10 * total_steps
                _miu_decay_end_iter = 0.20 * total_steps

                _t = (iteration - _miu_decay_start_iter) / max(
                    _miu_decay_end_iter - _miu_decay_start_iter,
                    1.0,
                )
                _t = max(0.0, min(_t, 1.0))

                miu = _miu_end + 0.5 * (_miu_start - _miu_end) * (
                    1 + math.cos(math.pi * _t)
                )
            ...
            if iteration % args.save_interval == 0 and iteration != starting_iteration:
                if (outer_shard or dp_rank == 0) and momentum_checkpoint_path is not None:
                    # momentum_to_save = [buf.cpu() for buf in momentum_buffer]
                    torch.save(checkpoint_outer_states(momentum_buffer), momentum_checkpoint_path)
                    print(f"dp{dp_rank},tp{tp_rank}, pp{pp_rank} saved to {momentum_checkpoint_path} at {iteration}")
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    length = len(string) + 1
    if rank == 0:
        print(f"The loss value is: {type(loss_value)}, {loss_value}")
    '''
    core_model = model[0].module.module # one module for ddp, one module for fp16
    m = hashlib.sha256()
    ...
    for name, param in core_model.named_parameters():
        param_fp32 = param.detach().to(dtype=torch.float32, device='cpu')
        m.update(param_fp32.numpy().tobytes())
    m = m.hexdigest()
    print(f"The rank is: {rank}, the check sum of forward model is: {m}")
    '''
