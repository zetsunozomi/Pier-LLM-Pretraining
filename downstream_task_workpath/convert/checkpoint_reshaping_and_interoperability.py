...
def safe_load_megatron_checkpoint(checkpoint_path):
    check_torch_load_is_safe()
    # Megatron checkpoints store training args plus optimizer/RNG metadata.
    # Those are not accepted by PyTorch's weights_only loader.
    ...
    for i in range(tp_size):
        ...
        for checkpoint_name in ["model_optim_rng.pt", "model_rng.pt"]:
            ...
            if os.path.isfile(checkpoint_path):
                break
        state_dict = safe_load_megatron_checkpoint(checkpoint_path)
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    state_dict = safe_load_megatron_checkpoint(rank0_checkpoint_path)
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Skip non-layer keys, e.g. final norm entries.
            if m is None:
                continue
            ...
            if op_name.endswith("layernorm") or op_name in ["input_norm", "post_attention_norm"]:
                ln_name = "ln_1" if op_name in ["input_layernorm", "input_norm"] else "ln_2"
    ...
    if "final_layernorm.weight" in params:
