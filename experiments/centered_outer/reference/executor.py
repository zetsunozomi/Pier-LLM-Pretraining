...
class CenteredExecutor:
    ...
    def __init__(self, master, reference, momentum, *, cohort, tile_elements,
        ...
        if tile_elements < 1 or master.numel() < 1:
        ...
        if any(x.dtype != torch.float32 or not x.is_contiguous()
               for x in (master, reference, momentum)):
        ...
        if any(x.device != master.device and x.device.type != 'cpu'
               for x in (reference, momentum)):
            raise ValueError('state must be on the master device or CPU')
        if any(overlaps(x, y) for x, y in ((master, reference), (master, momentum),
                                          (reference, momentum))):
        ...
        self.buffers = {name: torch.empty(size, dtype=torch.float32, device=master.device)
    ...
    def storage_tensors(self):
        return [self.master, self.reference, self.momentum, *self.buffers.values()] + (
    ...
    def step(self, *, mu=.9, eta=.7):
        ...
        if self.master.device.type == 'cuda':
            torch.cuda.synchronize(self.master.device)
