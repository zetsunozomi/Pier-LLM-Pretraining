...
# 1) 增加 1-GPU
gpu_labels = [1, 2, 4, 8, 16, 32]
x = list(range(len(gpu_labels)))

# --- 时间数据（秒） ---
# DiLoCo data
times_algo1_bars = np.array([np.nan, 827.935, 421.827, 301.842, 249.582, 195.469])
# DiLoCo data, but at gpu == 1 set to ddp
times_algo1_line = np.array([1661.1275862068965, 827.935, 421.827, 301.842, 249.582, 195.469])

# DDP
times_algo2 = np.array([1661.1275862068965,847.157, 464.337, 396.37, 243.95, 217.117])

# Momentum Produce
times_algo3 = np.array([np.nan,848.6416666666667, 457.48083333333335, 396.74333333333334, 216.37833333333333, 216.44083333333333])
...
plt.savefig("benchmark_small_vista.png", dpi=200)
