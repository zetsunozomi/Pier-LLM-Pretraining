...
ddp_light = '#5e95d0'
# 1. 定义 GPU 数量（可随时增删）
gpu_labels = [32, 64, 128]  # 如果以后要加 4、64，只要在这里改列表即可
# 2. 生成等距的 x 位置
...
import numpy as np
# 1: DiLoCo 
times_algo1 = np.array([445.63568749999996, 267.49793750000003, 249.6295])/1000
# 2 : DDP
times_algo2 = np.array([705.4825 , 550.1025, 529.165])/1000
# 3: MP
times_mp = np.array([1179.26, 573.8475, 568.42])/1000

times_algo1 = times_algo1*0.9 + times_mp*0.1
print(f"Pier:{times_algo1}")
print(f"ddp:{times_algo2}")

delta = times_algo2 - times_algo1
...
width = 0.20
offset = 0.1
bars1 = ax_time.bar([i - offset for i in x], times_algo1, width, label='Pier Time',color= diloco_light)
bars2 = ax_time.bar([i + offset for i in x], times_algo2, width, label='DDP time', color = ddp_light)
...
l4, = ax_eff.plot(x, eff_2, marker='v', linestyle='--',  linewidth=2, label='DDP efficiency', color = ddp_color)
...
plt.subplots_adjust(left=0.09,right=0.9,top=0.97,bottom=0.35)
plt.savefig("figure_500_v.png")
