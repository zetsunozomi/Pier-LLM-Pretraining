...
# 1. 定义 GPU 数量（可随时增删）
gpu_labels = [8, 16, 32]  # 如果以后要加 4、64，只要在这里改列表即可
# 2. 生成等距的 x 位置
...
import numpy as np
# 1: DiLoCo 
times_algo1 = np.array([518.8475, 279.1075, 155.2525])/1000
# 2 : DDP
times_algo2 = np.array([690.215, 445.615, 299.6825])/1000
# 3: MP
times_mp = np.array([695.955, 448.595, 400.205])/1000
...
plt.subplots_adjust(left=0.1,right=0.9,top=0.97,bottom=0.35)
plt.savefig("benchmark_small_perlmutter_fixed.png")
