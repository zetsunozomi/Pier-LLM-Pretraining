...
ddp_light = '#5e95d0'
# 1) 增加 1-GPU
gpu_labels = [1, 2, 4, 8, 16, 32, 64, 128, 256 ]
x = list(range(len(gpu_labels)))

# --- 时间数据（秒） ---
# DiLoCo data
times_algo1_bars = np.array([np.nan, 40528.439687499995, 21634.902968749997, 11246.22953125, 5895.2446875000005, 3022.716875, 1610.3567187499998, 905.4439062499999, 687.64640625])/1000
# DiLoCo data, but at gpu == 1 set to ddp
times_algo1_line = np.array([78029.7825, 40528.439687499995, 21634.902968749997, 11246.22953125, 5895.2446875000005, 3022.716875, 1610.3567187499998, 905.4439062499999, 687.64640625])/1000

# DDP
times_algo2 = np.array([78029.7825  ,40477.815, 21770.585, 13397.1575,8110.82,5711.935,4124.96,3377.8025, 2971.5])/1000

# Momentum Produce
times_algo3 = np.array([np.nan,39901.1725, 21688.3125 ,13715.315, 8542.35, 5569.115, 4205.195, 3756.2675, 3098.14])/1000
...
print(f"ddp iteration time {times_algo2}")
print(f"pier iteration time {times_algo1_bars}")
...
fig, ax_time = plt.subplots(figsize=(12,5.5))


# 绘制柱状图：时间（每次迭代）
width = 0.20
offset = 0.1
bars1 = ax_time.bar([i - offset for i in x], times_algo1_bars, width, label='Pier Time',color= diloco_light)
...
l3, = ax_eff.plot(x, eff_1, marker='^', linestyle='--', linewidth=2, label='Pier efficiency',color = diloco_color)
...
ax_eff.set_ylim(-20, 110)  # 百分比，适当留白
...
plt.subplots_adjust(left=0.14,right=0.9,top=0.97,bottom=0.35)
plt.savefig("figure_65_p_200.png")
